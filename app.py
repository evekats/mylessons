    ...
    if st.session_state.view_mode == 'list':
        ...
        with t2:
            with st.form("note_page", clear_on_submit=True):
                ...
                # Ημερομηνία σε μορφή DD/MM/YYYY
                ex_date = st.date_input("Διαγώνισμα", value=None, format="DD/MM/YYYY")
                uploaded_file = st.file_uploader("Αρχείο")
            ...
            # Σημειώσεις σε Expander για να μην πιάνουν χώρο
            st.subheader("Ιστορικό Σημειώσεων")
            for idx, nr in st.session_state.df_n[st.session_state.df_n['Μαθητής'] == sel].iloc[::-1].iterrows():
                # Το expander δείχνει την ημερομηνία και αν υπάρχει διαγώνισμα
                exam_label = ""
                if nr['Διαγωνίσματα'] and nr['Διαγωνίσματα'] != "":
                    try:
                        exam_date_f = datetime.strptime(nr['Διαγωνίσματα'], '%Y-%m-%d').strftime('%d/%m/%Y')
                        exam_label = f" | 🚨 Διαγώνισμα: {exam_date_f}"
                    except: pass
                
                with st.expander(f"📅 {nr['Ημερομηνία']}{exam_label}"):
                    st.write(nr['Σημειώσεις'])
                    if nr['Αρχείο']: st.link_button("📂 Αρχείο", nr['Αρχείο'])
                    if st.button("🗑️ Διαγραφή Σημείωσης", key=f"dn_{idx}"):
